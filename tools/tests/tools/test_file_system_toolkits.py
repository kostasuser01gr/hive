...
class TestWriteToFileTool:
    ...
    def test_write_overwrite_blocked_by_default(
        self, write_to_file_fn, mock_workspace, mock_secure_path, tmp_path
    ):
        """Writing to existing file is blocked by default."""
        test_file = tmp_path / "overwrite.txt"
        test_file.write_text("Original content")

        result = write_to_file_fn(
            path="overwrite.txt", content="New content", **mock_workspace
        )
    ...
    def test_write_explicit_overwrite(
        self, write_to_file_fn, mock_workspace, mock_secure_path, tmp_path
    ):
        """Writing to existing file succeeds with explicit overwrite=True."""
        test_file = tmp_path / "overwrite.txt"
        test_file.write_text("Original content")
